...
const _PARTS_PLAYERMAIN={
	...
	_start_shots(){
		let _this=this;
		//自機・オプションのショット
		_this._shots.shot[_this._shot_type].forEach((_ps,_i)=>{
			if(!_ps.player._isalive){return;}
			//ここで同時ショットと
			//個別ショットで判別させる
			if(_this._shot_type===_this._shot_type_def.DOUBLE){
				if(_ps.shots[0]._shot_alive||
					_ps.shots[1]._shot_alive){return;}
			...
			_ps._sq=(_ps._sq===_ps.shots.length-1)?0:_ps._sq+1;
			var _s=_ps.shots[_ps._sq];

			if(_s._shot_alive){return;}
			//ショット中は、ショットを有効にしない
			_s._shot=true;
	...
	_stop_shots(){
		this._shots.shot[this._shot_type].forEach((_ps) => {
			_ps.shots.forEach((_pss)=>{_pss._shot=false;});
		});
	}, //_move_shots_stop
	_start_missile_shots(){
		//自機・オプションのミサイルショット
		if(!this._shot_missle_isalive){return;}
		//ミサイル
		this._shots.missile.forEach((_pm,_i) => {
			if(!_pm.player._isalive){return;}
			//ここで同時ショット(2WAY)と
			//個別ショットで判別させる
			if(_PLAYERS_POWER_METER===3){
				if(_pm.shots[0]._shot_alive||
					_pm.shots[1]._shot_alive){return;}
	...
	_stop_missile_shots(){
		this._shots.missile.forEach((_pm)=>{
			_pm.shots.forEach((_pms)=>{_pms._shot=false;});
		})
	}, //_stop_missile_shots
	...
	_enemy_collision(_e){
		...
		for (let _i = _e.length-1; _i>=0; _i--) {
			...
			//ショットと敵の衝突判定
			_this._shots.shot[_this._shot_type].forEach((_os)=>{
			...
			//ミサイルと敵の衝突判定
			_this._shots.missile.forEach((_pm)=>{
				if (!_pm.player.isalive()) {return;}
				_pm.shots.forEach((_pms)=>{_pm.enemy_collision(_oe, _pms)});
			});
	...
	_init_laser_col_max(){
		if (!this._is_shottype_laser()){return;}
		this._shots.shot[this._shot_type].forEach((_os)=>{
			if (!_os.player._isalive) {return;}
			//レーザーのx位置も調整する。
			let _t = _os.shots[0];
	...
	_set_shot_laser_MaxX(){
		...
		this._shots.shot[this._shot_type].forEach((_os)=>{
			...
			//以下は敵衝突結果後とMAP衝突結果後を比べて、
			//レーザーのMAX値を求める
			if(_t._laser_col_max===0
				||_t._laser_col_max===null){
				//敵衝突がない場合。
...
class GameObject_SHOTS_MISSILE
	...
	constructor(_p){
		...
		//ミサイルの画像スプライトに対して、ミサイルのステータスと座標位置定義
...
//			console.log('_st3');
		...
		if(_this.get_missile_status(_t)==='_st1'){
			_map_y=_MAP.getMapY(_t.y+_this.imgsize+25);
			//壁中の時はリセットさせる
			if (_MAP.isMapCollision(_MAP.getMapX(_t.x), _MAP.getMapY(_t.y))) {
				_t._init();
				return;
			}
			...
			if(_MAP.isMapCollision(_map_x+1,_map_y)
				||_MAP.isMapCollision(_map_x,_map_y)){
				_this.set_missile_status(_t,'_st6');
				return;
			}
		}
...
class GameObject_SHOTS_NORMAL
	...
	map_collition(_t) {
		//MAPの位置を取得
		let _map_x = _MAP.getMapX(_t.x + (this._width / 2));
		let _map_y = _MAP.getMapY(_t.y + (this._height / 2));
	...
	move(){
		...
		for(let _j=0;_j<this.shots.length;_j++){
			...
			if(!_t._shot&&!_t._shot_alive){continue;}
			//撃ち始めは自機位置から放つ
			_t.x=(!_t._shot_alive)?_pl._x:_t.x+this.speed;
		...
		_this.set = _p.set || {
			...
			mapCol:[
				((_t)=>{
					...
					for (let _i = _gs.x - _t._set_speedX; _i < _gs.x; _i = _i + 5) {
						//MAPの位置を取得
				...
				((_t)=>{
					let _gs = this.getShotCenterPosition(_t)
					for (let _i = _gs.x - _t._set_speedX; _i < _gs.x; _i = _i + 5) {
					for (let _j = _gs.y + _t._set_speedY; _j >= _gs.y; _j = _j - 3) {
						//MAPの位置を取得
	...
	constructor(_p){
		...
		for(let _i=0;_i<1;_i++){
			_this.shots.push({
				...
				_is_col_map:false,//処理変数：MAP衝突有フラグ
				_shot:false,//処理変数：照射フラグ
				_shot_alive:false,//処理変数：照射中フラグ
