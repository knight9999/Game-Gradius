...
const _BS_SRC = require("browser-sync").create('server_dist');
...
const _PATH = process.argv[2] === 'src' ? './src' : './dist';
...
const _SET_JSON = () => {
    _FS.readdir('./src', function (_e, _f) {
        if (_e) throw _e;
        _f.forEach((_file) => {
            if (!_FS.statSync('./src/' + _file) || !(/.*\.json$/.test(_file))) {
                return;
            }
            if (/.*_editing_.*\.json$/.test(_file)) {
                return;
            }
